...
class EnhancedScreenCaptureService : Service() {
    ...
    companion object {
        ...
        const val TAG = "EnhancedScreenCapture"
    ...
    private var enhancedMatchLogic: EnhancedMatchLogic? = null
    ...
    private fun initializeRecognition() {
        ...
        enhancedMatchLogic = EnhancedMatchLogic()
    ...
    private fun performCardRecognition(screenshot: Bitmap) {
        ...
        val enhancedMatchLogic = this.enhancedMatchLogic ?: return
        ...
        try {
            ...
            // Update game logic
            val gameUpdate = enhancedMatchLogic.updateGameState(detectionResult)
            
            // Calculate next move
            val nextMove = enhancedMatchLogic.calculateNextMove()
            ...
            broadcastGameState(enhancedMatchLogic.getGameStats())
            
            // Check if game is complete
            if (enhancedMatchLogic.isGameComplete()) {
    ...
    private fun executeMove(move: EnhancedMatchLogic.NextMove) {
        when (move) {
            is EnhancedMatchLogic.NextMove.FLIP_CARD -> {
            ...
            is EnhancedMatchLogic.NextMove.MAKE_MATCH -> {
                // Tap first card
                ...
                captureHandler.postDelayed({
                    ...
                    enhancedMatchLogic?.recordMatch(move.card1, move.card2)
                }, 300)
    ...
    private fun broadcastGameState(stats: EnhancedMatchLogic.GameStats) {
        val intent = Intent(ACTION_AUTOMATION_STATE).apply {
